...
var init = () => {
    ...
    let c22Cost = new CustomCost((level) =>
        {
            var cost = 1;
            BigNumber.from(cost) = BigNumber.from(cost) * BigNumber.from(3.74447096981) ** level;
...
var tick = (elapsedTime, multiplier) => {
    ...
    currency2.value += dt * bonus * ((BigNumber.TWO * getC21(c21.level)) * (getC22(c22.level) / BigNumber.TWO)).pow(BigNumber.ONE + getC23(c23.level)); 
    }
    }
